...
async function _setupEndpoint({app, endpoint, bucketName = defaultBucketName}) {
  ...
  const bucket = new database.mongo.GridFSBucket(database.client, {
    bucketName
    // FIXME: add writeConcern, readPreference
  });
  ...
  await Promise.all([
    // public id lookup
    collection.ensureIndex('metadata.digestValue', {
      sparse: true,
      unique: true
    }),
    // use to GC of invalid files
    collection.ensureIndex({'metadata.valid': 1, 'metadata.created': 1}, {
      sparse: true
    }),
  ...
  function storeRawContent(req, res) {
  }

  function storeMultipartContent(req, res) {
    ...
    busboy.on('partsLimit', (ev) => {
      partsLimit = true;
    });
    busboy.on('filesLimit', (ev) => {
      // TODO
      filesLimit = true;
    });
    busboy.on('fieldsLimit', (ev) => {
      fieldsLimit = true;
    });
    busboy.on('finish', async () => {
      ...
      Promise.all(filePromises)
        .then(results => {
          ...
          return fileInfo.map(info => setGridFsMetadata(info._id, {
            valid: true
          }));
        })
        .then(results => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if(/^multipart\/form-data/.test(contentType)) {
        return storeMultipartContent(req, res);
      } else {
        // TODO: handle raw uploads (for easier API use vs browser use)
        throw new Error('Raw uploads unimplemented');
      }
    }));
