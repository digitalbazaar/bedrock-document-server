...
async function _setupEndpoint({app, endpoint}) {
  ...
  bucketName = 'fs_' + (endpoint.bucketName || defaultBucketName);
  ...
  function storeRawContent(req, res, next) {
    ...
    let stream;
    if(mimetype.startsWith('application/json') || mimetype.startsWith('application/ld+json')) {
      stream = new Readable();
  ...
  function storeMultipartContent(req, res) {
    ...
    busboy.on('file', (fieldname, file, filename, encoding, mimetype) => {
      ...
      const filePromise = Promise.all([gfsUpload, checksum])
        .then(results => {
          if(file.truncated) {
            throw new RangeError('File size too large.');
          }
          return setGridFsMetadata(results[0]._id, {
            // digest info
            digestAlgorithm,
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if(/^multipart\/form-data/.test(contentType)) {
        ...
        return storeRawContent(req, res);
      }
    }));
