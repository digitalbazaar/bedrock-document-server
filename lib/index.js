...
const sessionAuthentication = createMiddleware({
  required: true,
  optionsMap: {
    session: {
...
api.get = brCallbackify(async (actor, docId, options = {}) => {
  // FIXME: check permissions w/owner translation
  // FIXME: make `docId` full URL
  await checkPermission(
    actor, PERMISSIONS.DOCUMENT_ACCESS,
    {resource: docId});
...
async function _setupEndpoint({app, endpoint}) {
  ...
  app.post(
    ...
    asyncHandler(async (req, res, next) => {
      // FIXME: check permissions w/owner translation
      await checkPermission(req.user.actor, PERMISSIONS.DOCUMENT_CREATE);
  ...
  const requireOcap = brPassport.createMiddleware({
    ...
    optionsMap: {
      'http-signature-strategy': {
        async ocapToResourceRoles({capabilityInvocation}) {
          const resourceRoles = [];
          if(capabilityInvocation.action === 'ReadDocument') {
            // TODO: tighten up permissions in the future to be specific
            // to the owner of the doc, for now, if delegator is managed by
            // an account on the system, accept it
            let manager;
            for(const identity of capabilityInvocation.delegators) {
              manager = await brAccount.getManagerId(
                {actor: null, identity});
              if(manager) {
                break;
              }
            ...
            if(manager) {
              // FIXME: fix docIds to be full URLs
              const parts = capabilityInvocation.capability.invocationTarget
                .split('/');
              const docId = parts[parts.length - 1];
              resourceRoles.push({
                sysRole: 'document-server.reader',
                resource: docId
              });
            }
          }
      ...
      expressHttpProxy(url, {
        ...
        async proxyReqOptDecorator(proxyRequest, originalRequest) {
          ...
          if(!url.startsWith(baseUri)) {
            // TODO: be more strict and whitelist headers
            delete proxyRequest.headers.authorization;
            delete proxyRequest.headers.cookie;
            delete proxyRequest.headers['x-xsrf-token'];
          }
          if(plugin) {
