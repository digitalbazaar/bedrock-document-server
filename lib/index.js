...
api.use = (capabilityName, capabilityValue) => {
  if(capabilityValue) {
    if(!(typeof capabilityValue === 'object' &&
      ...
      typeof capabilityValue.api.modifyRequest === 'function')) {
...
bedrock.events.on('bedrock-express.configure.routes', async app => {
  ...
  app.get(
    ...
    asyncHandler(async (req, res, next) => {
      let plugin;
      const {url, pluginName} = req.query;
      ...
      if(Array.isArray(pluginName)) {
        ...
        plugin = await api.use(pluginName);
      ...
      expressHttpProxy(url, {
        ...
        async proxyReqOptDecorator(proxyRequest, originalRequest) {
          ...
          if(plugin) {
            await plugin.api.modifyRequest({proxyRequest, originalRequest});
          }
          return proxyRequest;
        },
